...
```
Get-DfsrBacklog [[-ComputerName] <String[]>] [[-FolderName] <String[]>]
```

## DESCRIPTION
This cmdlet queries the specified computer and its replication parteners for DFS replication senarios and their current inbound replication backlog. 
The connection are made using CIM and WMI.
...
### -ComputerName
The name of the computer to query for DFS replication senarios. 
This defaults to the local machine if no value is given.
...
If no value is provided, then all folders are returned.
...
[Online Version:](http://go.microsoft.com/fwlink/?LinkID=280068)
