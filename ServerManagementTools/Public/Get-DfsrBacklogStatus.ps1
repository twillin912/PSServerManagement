function Get-DfsrBacklogStatus {
    ...
    Begin {
        $CimFallbackOptions = New-CimSessionOptions -Protocol 'DCOM'
        $Output = @()
    }
    ...
    Process {
        foreach ( $Computer in $ComputerName ) {
            ...
            try {
                ...
                # $WmiFailback = $true
                $CimSession = New-CimSession -ComputerName $Computer -SessionOptions $CimFallbackOptions -ErrorAction Stop
            }


            $DfsrConnInfo = Get-CimInstance -CimSession $CimSession -Namespace 'root\MicrosoftDFS' -ClassName 'DfsrConnectionInfo' -ErrorAction Stop
            $DfsrFolderInfo = Get-CimInstance -CimSession $CimSession -Namespace 'root\MicrosoftDFS' -ClassName 'DfsrReplicatedFolderInfo' -ErrorAction Stop
